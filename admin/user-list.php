<?php
  ...
  $start = 0;
  $rowsPerPage = 5;
  $totalPages = ceil(count($totalUserList) / $rowsPerPage);
  ...
  if(isset($_POST['search']) || isset($_COOKIE['search'])){
    ...
    $searchPost = $table->getSearchPost($searchKey);
    $totalPages = ceil(count($searchPost) / $rowsPerPage);
    
    if(isset($_GET['page-no'])){
      $start = ($_GET['page-no'] - 1) * $rowsPerPage;
      $totalUser = $table->getSearch($searchKey, $start, $rowsPerPage);
    }else{
      $totalUser = $table->getSearch($searchKey, $start, $rowsPerPage);
    }
  }else{
    if(isset($_GET['page-no'])){
      $start = ($_GET['page-no'] - 1) * $rowsPerPage;
      $totalUser = $table->pagination($start, $rowsPerPage);
    }else{
      $totalUser = $table->pagination($start, $rowsPerPage);
    }
  }
      ...
      <div class="row">
        ...
        <div class="col">
          ...
          <div class="action-btn text-end">
            <a href="add-user.php" class="btn btn-primary mb-2">Add User</a>
          </div>
          ...
          <div class="card">
            <div class="card-body p-4">
              <div class="table-responsive">
                  ...
                  if (!$totalUser) {
                   ?>
                    <p class="text-center card-title m-0">No Found User</p>
                   <?php
                  }else{ ?>
